...
A regulated underwriting service is judged on operational health, not just model
quality. This is a lightweight, dependency-free metrics collector that the API
middleware feeds and the monitoring endpoint reads. It keeps a bounded rolling
window of recent request latencies (constant memory) plus running counters, and
exposes p50/p95/p99 latency, error rate, and throughput.

Thread-safe. Process-local by design — a multi-replica deployment would export
these to Prometheus / OpenTelemetry; the collection points are identical. Mirrors
FraudPulse's metrics module so both portfolio apps share one observability method.
"""
